...
static ota_err_t ota_get_next_update_partition()
{
  ...
  {
    ESP_LOGE(TAG,"esp_ota_get_next_update_partition ?");
    return OTA_RESULT_FAIL;
  }

  ESP_LOGI(TAG,"update_partition size 0x%x subtype %d at offset 0x%x",
...
static ota_err_t ota_verify_update_partition_checksum()
{
  ...
  for(size_t offset = 0; offset < _vars.update_bin_size; )
  {
    ...
    ESP_LOGD(TAG, "md5 offset 0x%x | block_len 0x%x", offset, block_len);
    esp_err_t err = esp_partition_read(_vars.update_partition, offset, buf, block_len);
    if(err != ESP_OK)
    {
      ESP_LOGE(TAG,"esp_ota_read failed! err=0x%x", err);
      return OTA_RESULT_FAIL;
    }
  ...
  {
    ESP_LOGE(TAG,"md5 checksum mismatch %s != %s", md5str, _vars.fetched_params.checksum);
    return OTA_RESULT_FAIL;
  }
...
static char* concat_path(const char* left, const char* right)
{
  ...
  if(len < 0)
  {
    ESP_LOGE(TAG,"concat_path failed");
    return NULL;
  }
  return dest;
}
...
static ota_err_t download_to_update_partition(esp_http_client_config_t *config)
{
  ...
  if (err != ESP_OK)
  {
    ESP_LOGE(TAG, "esp_http_client_open failed, error=0x%x", err);
    goto error;
  }
...
static ota_err_t parse_params(const char* str)
{
  ...
  if(json == NULL)
  {
    ...
    if(error != NULL)
    {
      ESP_LOGE(TAG,"parse %s", error);
    }
    else
    {
      ESP_LOGE(TAG,"failed to get error message");
    }
    goto error;
  }
  ...
  {
    ESP_LOGE(TAG,"can't get path value");
    goto error;
  }
  ...
  if(!cJSON_IsString(item) || item->valuestring == NULL)
  {
    ESP_LOGE(TAG,"can't get checksum value");
    goto error;
  }
  ...
  {
    ESP_LOGE(TAG,"can't get version value");
    goto error;
  }
  ...
  if(!cJSON_IsNumber(item))
  {
    ESP_LOGE(TAG,"can't get interval value");
    goto error;
  }
