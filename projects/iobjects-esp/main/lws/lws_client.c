...
static const char* TAG = "lws_client";
...
void lws_client_set_login_host_port(const char* host_port)
{
  ...
  if(host_port && *host_port)
  {
    ...
    {
      ESP_LOGE(TAG, "asprintf error in lws_client_set_login_host_port");
    }
  }

  ESP_LOGW(TAG, "switching login url to %s",
...
void lws_client_set_websocket_host_port(const char* host_port)
{
  ...
  if(host_port && *host_port)
  {
    ...
    {
      ESP_LOGE(TAG, "asprintf error in lws_client_set_websocket_host_port");
    }
  }

  ESP_LOGW(TAG, "switching ws url to %s",
...
static void client_receive_authkey_handler(char *data, size_t size)
{
  ...
  if(root == NULL)
  {
    ...
    if(error != NULL && *error)
    {
      ESP_LOGE(TAG, "[JSON] %s", error);
    }
    else
    {
      data[size-1] = 0;
      ESP_LOGE(TAG, "[JSON] failed to get error message for:\n%s", data);
    }
    return;
  }
  ...
  if(authdata.error_code == 0)
  {
    ...
    if(child == NULL)
    {
      ...
      {
        ESP_LOGE(TAG, "[JSON] failed to get error message");
      }
    }
  ...
  {
    ESP_LOGI(TAG, "authdata.data.device_id: %s", authdata.data.authkey);
  }
  else
  {
    ESP_LOGE(TAG, "[JSON] failed to get authkey");
    ESP_LOGE(TAG, "%s", json_data);
  }
...
static int client_login()
{
  ...
  if(body.ok != true)
  {
    ESP_LOGE(TAG, "failed to get request body");
    goto error;
  }
  ...
  {
    ESP_LOGE(TAG, "suspicious response size %u", read_length);
    goto error;
  }
...
void lws_client_task(void *pvParameters)
{
  ...
  {
    ESP_LOGE(TAG, "lwsClient:websocket transmit data queue not created");
    vTaskDelete(NULL);
    return;
  }
    ...
    if(wsi != NULL)
    {
      ...
      while(!lws_service(context, 10))
      {
        if(event_wait_websocket_client_connected_to_server_timeout(0))
        {
          ...
          if(xStatus == pdPASS)
          {
            ESP_LOGI(TAG, "ws tx: %s", item.data);
            size_t out_len;
            ...
            if(out_len)
            {
              ...
              {
                ESP_LOGE(TAG, "ignore lws write too small buffer?");
              }
              else
              {
            ...
            {
              ESP_LOGE(TAG, "base64_encode error");
            }
            free(enc);
          }
        }
  ...
  ESP_LOGI(TAG, "lws_client_task deleted");
  vTaskDelete(NULL);
}
