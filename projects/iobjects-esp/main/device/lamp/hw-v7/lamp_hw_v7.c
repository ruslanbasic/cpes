...
{
  nvs_set_integer(NVS_INT_LAMP_BRIGHTNESS, 50);
  nvs_set_integer(NVS_INT_LAMP_COLOR_TEMPERATURE, 50);
}
...
void lamp_task(void *pvParameters)
{
  ...
  userBrightness = nvs_get_integer(NVS_INT_LAMP_BRIGHTNESS);
  userBalance = nvs_get_integer(NVS_INT_LAMP_COLOR_TEMPERATURE);
  ...
  const debounce_nvs_handle_t debounce_brigh = debounce_nvs_create(
        pdMS_TO_TICKS(4000), userBrightness, NVS_INT_LAMP_BRIGHTNESS);
  if(debounce_brigh == NULL)
  {
    ESP_LOGE(TAG, "could not allocate memory for debounce_brigh");
    vTaskDelete(NULL);
    return;
  }

  const debounce_nvs_handle_t debounce_blnc = debounce_nvs_create(
        pdMS_TO_TICKS(4000), userBalance, NVS_INT_LAMP_COLOR_TEMPERATURE);
  if(debounce_blnc == NULL)
  {
    ESP_LOGE(TAG, "could not allocate memory for debounce_blnc");
    vTaskDelete(NULL);
    return;
  }
