...
static const char* LOG_TAG = "[autoupdate]";
...
{
  ESP_LOGW(LOG_TAG, "next check after %u seconds", sleep);
  if(event_wait_urgent_ota_update_or_timeout(pdMS_TO_TICKS(1000 * sleep)))
  {
    ESP_LOGW(LOG_TAG, "urgent update started");
...
void auto_update_set_host_port(const char* host_port)
{
  ...
  if(host_port && *host_port)
  {
    ...
    if(ret == -1)
    {
      ESP_LOGE(LOG_TAG, "asprintf error in ota_set_google_host");
    }
  }
  ESP_LOGW(LOG_TAG, "switching url to %s", update_server_url ? update_server_url : "default");
...
void auto_update_task(void *pvParameters)
{
  ...
  for(;;)
  {
    ...
    {
      ESP_LOGW(LOG_TAG, "Updating from %u to %u ...", SW_VERSION, latest);
      device_set_state(eOtaUpdateStarted);
      if(download_and_install() == OTA_RESULT_OK)
      {
        publish(TO_SERVER_AUTOUPDATE, "");
        ESP_LOGW(LOG_TAG, "Prepare to restart system after 10 seconds!");
        vTaskDelay(pdMS_TO_TICKS(10000));
        esp_restart();
      }
    ...
    {
      ESP_LOGI(LOG_TAG, "no need to update %u", SW_VERSION);
    }
