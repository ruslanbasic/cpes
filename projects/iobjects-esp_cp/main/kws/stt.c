...
static bool stt_parse_impl(const char* buf)
{
  ...
  if(json == NULL)
  {
    ...
    if(error_ptr != NULL)
    {
      ESP_LOGE(LOG_TAG, "Error before: %s\n", error_ptr);
    }
    else
    {
      ESP_LOGE(LOG_TAG, "can't parse json");
    }
    goto end;
  }
  ...
  {
    ESP_LOGE(LOG_TAG, "can't take result object from json");
    goto end;
  }
  ...
  cJSON_ArrayForEach(result, results)
  {
    ...
    {
      ESP_LOGE(LOG_TAG, "can't take alternative object from json");
      goto end;
    }
    ...
    cJSON_ArrayForEach(alternative, alternatives)
    {
      ...
      if(transcript == NULL)
      {
        ESP_LOGE(LOG_TAG, "can't take transcript object from json");
        goto end;
      }
      ...
      {
        // ESP_LOGI(LOG_TAG, "speech transcript %s", transcript->valuestring);
      ...
      {
        ESP_LOGE(LOG_TAG, "invalid type of transcript object from json");
        goto end;
      }
    }
  }
...
void stt_google_parse(const char* buf)
{
  ...
  if(!any_action)
  {
    ESP_LOGW(LOG_TAG, "no actions found in json\n%s", buf1);
  }
}
...
void stt_set_google_host_port(const char* host_port)
{
  ...
  if(host_port && *host_port)
  {
    ...
    if(ret == -1)
    {
      ESP_LOGE(LOG_TAG, "asprintf error in stt_set_google_host");
    }
  }
  ESP_LOGW(LOG_TAG, "switching url to %s", google_url ? google_url : "default");
}
