...
#error forgot #include configuration.h ?
...
#if DEVICE == DEVICE_SOCKET && HW_VERSION == 5
  ...
  {
    error("device: xCurrentStateMutex not created");
  }
...
static void nvs_update_switch_on_counter_task(void *pvParameters)
{
  ...
  switch_on_count = nvs_get_switch_on_counter();
  nvs_set_switch_on_counter(switch_on_count + 1);

  for(;;)
  {
    vTaskDelay(5000 / portTICK_PERIOD_MS);
    nvs_reset_switch_on_counter();
    break;
  }
  vTaskDelete(NULL);
}
...
void buttons_task(void *pvParameters)
{
...
#elif (DEVICE == DEVICE_SOCKET) || (DEVICE == DEVICE_HEATER)
  ...
  for(;;)
  {
    ...
    {
      nvs_set_switch_on_counter(3);
      warning("!!!!!!!! RESTARTING ON USER BUTTON (3) !!!!!!!!");
      esp_restart();
    }
...
void device_task(void *pvParameters)
{
  ...
  info("free heap memory: %d bytes", esp_get_free_heap_size());
  info("idf version: %s", esp_get_idf_version());
  info("%s hw version: %d.%d.%d sw version: %d.%d.%d",
...
device_mode_t device_get_mode()
{
  ...
  wifi_config_t info;
  esp_err_t error = nvs_get_user_router_info(&info);

#if DEVELOPERS_ONLY_PREDEFINED_WIFI == 1
  if(error != ESP_OK)
  {
    warning(
      "\n\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n"
      "Developers mode ! Predefined WIFI !\n"
      "Remove me from release please ! \n"
      "See DEVELOPERS_ONLY_PREDEFINED_WIFI\n"
      "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    nvs_set_user_router_info(&(user_router_info_t)
    {
      .ssid = DEVELOPERS_ONLY_PREDEFINED_WIFI_SSID,
      .pass = DEVELOPERS_ONLY_PREDEFINED_WIFI_PASS
    });
    return mode;
  }
#endif
  ...
  if((switch_on_count >= 3u) || (error != ESP_OK))
  {
...
http_body_t device_get_handshake_body()
{
  ...
  {
    error("size of handshake body buffer is not enough");
    body.ok = false;
    return body;
  }
  ...
  {
    error("mbedtls_base64_encode problem");
    body.ok = false;
    return body;
  }
  ...
  {
    error("size of handshake buffer is not enough");
    body.ok = false;
  }
  ...
  else
  {
    ...
    if(!stash)
    {
      switch(device_state)
      {
        ...
        case eConfiguring:            rgb_set_color(eBlue); stash = true; break;
