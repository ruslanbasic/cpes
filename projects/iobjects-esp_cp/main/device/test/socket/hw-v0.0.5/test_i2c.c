 ...
 * - master:
 *    GPIO19 is assigned as the data signal of i2c master port
 *    GPIO4 is assigned as the clock signal of i2c master port
 *
 * Connection:
 *
 * - connect GPIO19 with GPIO25
 * - connect GPIO4 with GPIO26
...
#define I2C_EXAMPLE_MASTER_SCL_IO          TEST_I2C_MASTER_SCL_IO               /*!< gpio number for I2C master clock */
#define I2C_EXAMPLE_MASTER_SDA_IO          TEST_I2C_MASTER_SDA_IO               /*!< gpio number for I2C master data  */
 ...
 *
 */
static esp_err_t i2c_example_master_write_slave(i2c_port_t i2c_num,
 ...
 * --------|---------------------------|--------------------|--------------------|------|
 */
static esp_err_t i2c_example_master_sensor_test(i2c_port_t i2c_num,
...
static void i2c_example_slave_init()
{
  ...
  i2c_driver_install(i2c_slave_port, conf_slave.mode,
                     I2C_EXAMPLE_SLAVE_RX_BUF_LEN, I2C_EXAMPLE_SLAVE_TX_BUF_LEN, 0);
}
...
static void i2c_test_task(void* arg)
{
  ...
  {
    printf("test cnt: %d\n", cnt++);
    ret = i2c_example_master_sensor_test( I2C_EXAMPLE_MASTER_NUM,
    ...
    xSemaphoreTake(print_mux, portMAX_DELAY);
    size_t d_size = i2c_slave_write_buffer(I2C_EXAMPLE_SLAVE_NUM, data,
                                           RW_TEST_LENGTH, 1000 / portTICK_RATE_MS);
    if(d_size == 0)
    {
      printf("i2c slave tx buffer full\n");
      ret = i2c_example_master_read_slave(I2C_EXAMPLE_MASTER_NUM, data_rd,
                                          DATA_LENGTH);
    }
    else
    {
    ...
    if(ret == ESP_OK)
    {
      size = i2c_slave_read_buffer( I2C_EXAMPLE_SLAVE_NUM, data, RW_TEST_LENGTH,
...
TEST_CASE("i2c", TAG)
{
  printf("%u <=> %u\n", I2C_EXAMPLE_MASTER_SCL_IO, 26);
  printf("%u <=> %u\n", I2C_EXAMPLE_MASTER_SDA_IO, 25);
