...
static error_counter_handle_t error_counters[eSensorMAX];
...
static TimerHandle_t xTimerSmoothHeatingOn, xTimerSmoothHeatingOff;
static SemaphoreHandle_t xSmoothHeatingMutex;
static boolean heating_state;
...
static inline void enable_heating_gpio()
{
...
static inline void enable_heating()
{
  ...
  do
  {
    if((takeRes = xSemaphoreTake(xSmoothHeatingMutex, pdMS_TO_TICKS(42))) == pdTRUE)
    {
      enable_heating_gpio();
      heating_state = true;
      xSemaphoreGive(xSmoothHeatingMutex);
    }
    else
    {
      warning("xSmoothHeatingMutex take timeout");
    }
  }
  while(takeRes == pdFALSE);

  xTimerStop(xTimerSmoothHeatingOff, 0);
  xTimerReset(xTimerSmoothHeatingOn, 0);
}

/*****************************************************************************/

static inline void disable_heating_gpio()
{
  const consumption_data_t event =
  {
...
static inline void disable_heating()
{
  ...
  xTimerStop(xTimerSmoothHeatingOff, 0);
  xTimerStop(xTimerSmoothHeatingOn, 0);
}

/*****************************************************************************/

static void smooth_heating_on_handler( TimerHandle_t pxTimer )
{
  info("hello smooth on expired");

  xSemaphoreTake(xSmoothHeatingMutex, 0);
  {
    if(heating_state)
    {
      disable_heating_gpio();
  ...
  xTimerReset(xTimerSmoothHeatingOff, 0);
}

/*****************************************************************************/

static void smooth_heating_off_handler( TimerHandle_t pxTimer )
{
  info("hello smooth off expired");
...
static void queue_add(sensor_t sensor, float value)
{
  ...
  assert(sensor < eSensorMAX);
  const error_counter_handle_t err_handler = error_counters[sensor];

  if(value != -1000)
  {
    xQueueSend(xSensorsDataQueue, &item, 0);
    error_counter_succeed(err_handler);
  }
  else
  {
    error_counter_failed(err_handler);
  }

  if(error_counter_limit_exceeded(err_handler))
  {
    error_counter_reset(err_handler);
...
static void power_consumption_task(void *pvParameters)
{
  ...
  uint32_t consumed32 = nvs_power_consumption_load();
  ...
  for(;;)
  {
    ...
    if(status == pdPASS)
    {
      ...
      if(item.event == ePowerConsumptionEvtPush)
      {
        ...
        {
          consumed32 = temp;
          nvs_power_consumption_save_and_rotate(consumed32);
        }
      }
    }
  }
...
device_error_t device_control(device_method_t method, char *parameters)
{
  ...
  switch(method)
  {
    ...
    {
      // TODO: current server implemntation has race condition to set temperature
//      bool value = (bool)atol((const char *)parameters);
//      if(value == 1)
//      {
//        queue_add(eUserPhoneTemp, MAX_USER_TEMP);
//      }
//      else
//      {
//        queue_add(eUserPhoneTemp, MIN_USER_TEMP);
//      }
      break;
    }
...
static void storage_task(void *pvParameters)
{
  ...
  userTemp = nvsUserTemp = nvs_get_uint(NVS_NAMESPACE_DEVICE_STATE, "user_temp");
  queue_add(eBootTemp, userTemp);

  nvsGlassMaxTemp = glassMaxTemp = nvs_get_uint_or_default(NVS_NAMESPACE_DEVICE_STATE, "user_glass_temp", MAX_GLASS_TEMP);
  ...
  for(;;)
  {
        ...
        xTaskGetTickCount() - xNvmUserTempTicks > pdMS_TO_TICKS(4000))
    {
      ...
      nvs_set_uint(NVS_NAMESPACE_DEVICE_STATE, "user_temp", nvsUserTemp);
    ...
    {
      nvsGlassMaxTemp = glassMaxTemp;
      nvs_set_uint(NVS_NAMESPACE_DEVICE_STATE, "user_glass_temp",
                   nvsGlassMaxTemp);
...
static void dispath_sensor_error(sensor_t sensor)
{
  switch(sensor)
  {
    case eMlx90614RoomTemp:
      error("Sensor eMlx90614RoomTemp sent an error status");
      publish_error(ERROR_ROOM_TEMPERATURE_SENSOR_IS_BROKEN);
      break;
    case eGlassTemp:
      error("Sensor eGlassTemp sent an error status");
      publish_error(ERROR_GLASS_TEMPERATURE_SENSOR_IS_BROKEN);
      break;
    case eBme280Pressure:
      error("Sensor eBme280Pressure sent an error status");
      break;
    case eBme280Humidity:
      error("Sensor eBme280Humidity sent an error status");
      publish_error(ERROR_GLASS_HUMIDITY_AND_PRESSURE_SENSOR_IS_BROKEN);
      break;
    default:
      error("Sensor %d sent an error status", sensor);
      break;
  }
}
...
void heater_task(void *pvParameters)
{
  ...
  for(int i=0; i < eSensorMAX; i++)
  {
    error_counters[i] = error_counter_create(ERROR_COUNTER_LIMIT);
    assert(error_counters[i]);
  }

  xTimerSmoothHeatingOn = xTimerCreate("Timer_smooth_heating_on",
                                       pdMS_TO_TICKS(HEATING_SMOOTH_ON_TIME_MS),
                                       pdFALSE,        // auto-reload - false;
                                       ( void * ) 0,
                                       smooth_heating_on_handler);

  if(xTimerSmoothHeatingOn == NULL)
  {
    error("xTimerSmoothHeatingOn not created");
    vTaskDelete(NULL);
    return;
  }

  xTimerSmoothHeatingOff = xTimerCreate("Timer_smooth_heating_off",
                                        pdMS_TO_TICKS(HEATING_SMOOTH_OFF_TIME_MS),
                                        pdFALSE,        // auto-reload - false;
                                        ( void * ) 0,
                                        smooth_heating_off_handler);

  if(xTimerSmoothHeatingOff == NULL)
  {
    error("xTimerSmoothHeatingOff not created");
    vTaskDelete(NULL);
    return;
  }

  xSmoothHeatingMutex = xSemaphoreCreateMutex();
  if(xSmoothHeatingMutex == NULL)
  {
    error("xSmoothHeatingMutex not created");
    vTaskDelete(NULL);
    return;
  }
  ...
  for(;;)
  {
    ...
    if(status == pdPASS)
    {
      ...
      switch(item.sensor)
      {
        ...
        case eSensorError:
        {
           ...
           * */
          dispath_sensor_error((sensor_t)item.value);
          break;
        }
                            ...
                            current.user_temp > current.mlx90614_room_temp && current.mlx90614_glass_temp < current.user_glass_temp)
    {
      if(!current.heating)
      {
        ...
        warning("heating started ! [user_temp %u, room_temp %2.1f, user_glass_temp %u, glass_temp %2.1f]",
    ...
    else
    {
      if(current.heating)
      {
        ...
        warning("heating stopped ! [user_temp %u, room_temp %2.1f, user_glass_temp %u, glass_temp %2.1f]",
                current.user_temp, current.mlx90614_room_temp,  current.user_glass_temp, current.mlx90614_glass_temp);
      }
    }

    {
      ...
      {
        tmp = 0;
        warning("front_glass_temp %d", current.front_glass_temp);
        warning("back_glass_temp %d", current.back_glass_temp);

        warning("[heating %u, user_temp %u, room_temp %2.1f, user_glass_temp %u, glass_temp %2.1f]",
                current.heating, current.user_temp, current.mlx90614_room_temp,  current.user_glass_temp, current.mlx90614_glass_temp);
      }
    }
