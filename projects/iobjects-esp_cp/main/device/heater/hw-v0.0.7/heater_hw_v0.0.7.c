  ...
  for(;;)
  {
    ...
    if(status == pdPASS)
    {
      ...
      if(item.event == ePowerConsumptionEvtPush)
      {
        ...
        publish(TO_HEATER_POWER_CONSUMED, (const char *)data);
...
{
  return f1 * (9e-7 * pow(t1, 4) - 1e-5 * pow(t1, 3) + 2.1 * 1e-3 * pow(t1, 2) + 0.0499 * t1 + 0.5064)
         / (9e-7 * pow(t2, 4) - 1e-5 * pow(t2, 3) + 2.1 * 1e-3 * pow(t2, 2) + 0.0499 * t2 + 0.5064);
}
...
void heater_task(void *pvParameters)
{
  ...
  xTaskCreate(&lmt01_task,    "lmt01",    2048, NULL, 10, NULL);
    ...
    if(status == pdPASS)
    {
      ...
      switch(item.sensor)
      {
        ...
        case eUserEncoderTemp:
        {
          ...
          publish(TO_HEATER_USER_ROOM_TEMPERATURE, (const char *)data);
          break;
        }
        case eUserGlassMaxTemp:
        {
        ...
        case eMlx90614RoomTemp:
        {
          ...
          publish(TO_HEATER_REAL_ROOM_TEMPERATURE, (const char *)data);
          break;
        }
        case eBme280Temp:
        {
        ...
        case eBme280Pressure:
        {
          ...
          publish(TO_HEATER_REAL_ROOM_PRESSURE, (const char *)data);
          break;
        }
        case eBme280Humidity:
        {
          if(current.mlx90614_room_temp_is_set && current.bme280_temp_is_set)
          {
            ...
            if(f2 >= 0 && f2 <= 100)
            {
              ...
              publish(TO_HEATER_REAL_ROOM_HUMIDITY, (const char *) data);
            }
