...
static debounce_nvs_handle_t debounce_nvs;

static const char* LOG_TAG = "[rgbled]";
...
void rgb_initialize(rgb_gpio_t rgb_gpio, rgb_channel_t rgb_channel)
{
  ...
  total_brightness = nvs_get_uint_or_default(
                       NVS_NAMESPACE_DEVICE_STATE,
                       "rgb_brightness",
                       RGB_LED_MAX_TOTAL_BRIGHTNESS);

  debounce_nvs = debounce_nvs_create(pdMS_TO_TICKS(4000),
                                     total_brightness,
                                     NVS_NAMESPACE_DEVICE_STATE,
                                     "rgb_brightness");
  assert(debounce_nvs);
...
void rgb_set_total_brightness(uint8_t value)
{
  xSemaphoreTake(xRgbLedMutex, portMAX_DELAY);
  {
    ...
    debounce_nvs_update(debounce_nvs, total_brightness);
...
void rgb_set_color_blink(rgb_color_t color)
{
  ...
  xSemaphoreTake(xRgbLedMutex, portMAX_DELAY);
  {
    if (blink_active == false || lastColor != color)
    {
      ...
      {
        ESP_LOGE(LOG_TAG, "unsuccessful pBlinkTimer reset");
      }
    }
  }
  xSemaphoreGive(xRgbLedMutex);
...
static void blink_timer(TimerHandle_t pTimer)
{
  ...
  {
    ESP_LOGW(LOG_TAG, "can't blink: mutex timeout");
  }
}
