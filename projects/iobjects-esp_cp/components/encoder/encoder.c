...
static const char* LOG_TAG = "[encoder]";
  ...
  if(xValueMutex == NULL)
  {
    taskENTER_CRITICAL(&xLazyMux);
    {
      if(xValueMutex == NULL)
      {
        ...
        if(xValueMutex == NULL)
        {
          ESP_LOGE(LOG_TAG,"xValueMutex not created");
        }
      }
    }
    taskEXIT_CRITICAL(&xLazyMux);
  }
}
...
bool encoder_initialize(const encoder_options_t * const optionsPtr)
{
  ...
  {
    ESP_LOGE(LOG_TAG,"xGpioEventQueue not created");
    return true;
  }
      ...
      if(state.prev != state.next)
      {
        xSemaphoreTake(xValueMutex, portMAX_DELAY);
        {
          if((state.prev == 1) && (state.next == 2))
          {
            ...
            {
              counter.value++;
              ESP_LOGI(LOG_TAG, "encoder %d", counter.value);
              break;
            }
          }
...
rgb_brightness_t encoder_to_rgb(uint8_t value)
{
  ...
  {
    // warning("encoder value out of range %u", value);
