...
static const char* LOG_TAG = "[kws_recorder]";
static Fvad *p_vad;
...
static int recorder_wait_unvoice(audio_sample_t* const data, const size_t total)
{
  ...
  do
  {
    ...
    if(vadres < 0)
    {
      ESP_LOGE(LOG_TAG, "VAD processing failed");
      return 0;
    }
